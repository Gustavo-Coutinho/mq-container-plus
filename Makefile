...
downloads/$(MQ_ARCHIVE_DEV): $(MQ_ARCHIVE_DEV)
	...
	@if [ "$(MQ_ARCHIVE_DEV_EXISTS)" = "$(MQ_ARCHIVE_DEV)" ]; then \
		echo "file exists, do nothing"; \
	else \
		echo "file does not exist, so download it:"; \
		echo $(SPACER)$(shell printf $(TITLE)"Downloading IBM MQ Advanced for Developers "$(MQ_VERSION)$(END)); \
		mkdir -p downloads; \
		if [ "$(BUILD_RSYNC_SERVER)" != "$(EMPTY)" ]; then \
			...
			if [ "$(MQ_ARCHIVE_REPOSITORY_DEV)" != "$(EMPTY)" ]; then \
